...
```{r}
Year <- as.factor(as.character(number_of_passengers$Year))
...
Month <- number_of_passengers %>% 
  mutate(Month = as.integer(as.factor(as.character(number_of_passengers$Month))))

Month<-month.abb[number_of_passengers$Month] #STILL not working

number_of_passengers %>%
  filter(Year %in% c("2019", "2020")) %>% 
  mutate(year_sel = ifelse(Year < 2020, 
                           "2019", "2020")) %>% 
  ggplot(aes(x = MonthAbb, 
             y = Total,
             color = year_sel)) +
  geom_point() + 
...
unrate_large %>% 
  ...
  ggplot(aes(x = DATE, y = UNRATE, color = year_sel)) + 
  geom_line() +
  scale_color_manual(values = c("red", "blue")) +
  labs(color = "Year", 
       y = "Unemployment Rate (%)",
